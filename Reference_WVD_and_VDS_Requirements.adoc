---
sidebar: sidebar
permalink: WVD_and_VDS_requirements.html
summary: WVD and VDS Requirements
---

= WVD and VDS Requirements
...
* Refer to the link:WVD_and_VDS_components_and_permissions.html[WVD VDS Components and Permissions] document for an explanation of the required permissions and components in both Azure and the local Active Directory environment
...
VDS components in the Azure subscription communicate with the VDS global control plane components that are hosted in Azure, including the VDS Web Application and the VDS API endpoints. For access, the following base URI addresses need to be whitelisted for bi-directional access on port 443:
