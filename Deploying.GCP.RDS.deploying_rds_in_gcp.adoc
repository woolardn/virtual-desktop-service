...
==== Public domain
External access is secured via an SSL certificate.  This can be customized with your own domain and a self-managed SSL certificate. Alternativly, selecting _Automatic_ allows VDS to manage the SSl certificate including an automatic 90-day refresh of the certificate.  When using automatic, each deplpoyment uses a unique sub-domain of _cloudworkspace.com_.
...
You'll receive multiple emails as the deployment completes.  Once complete you'll have an environment ready for your first workspace.  A workspace will contain the session hosts and data servers needed to support the end users.  Come back to this guide to follow the next steps once the deployment automation completes in 1-4 hours.

== Create a new provisioning collection
Provisioning collections if the functionality in VDS that allows for the creation, customization and SysPrep of VM images.  Once we get into the workplace deployment, we'll need an image to deploy.
...
. Enter a Name and Description.  Choose _Type: VDI_.
. Click _Add_ to define the type of server image to build.
+
image:GCP_Deploy28.png[]

. Select the appropriate VM image (Server 2016 in this case) and the desired storage type. Click _Add Server_
+
...
In a multi-tenant deployment, VDS has been designed to fully isolate each of these workspaces fully such at different organizations could be hosted together within a single deployment.  They would share gateway and other platform services (reducing overhead costs)while maintaining their own dedicated sessions hosts, data layer and data/app servers. The same concept can be applied to distinct divisions within a single organization, allowing for efficiency in a shared domain while securely isolating unrelated departments within the same organization.
...
. Select an existing client or click _Add_ to create a new client.
+
image:GCP_Deploy26.png[]

.. If adding a new client, enter company details and select the deployment into which this workspace will be deployed.
...
... _Enable Workspace User Data Storage:_ Determine if end users have a need to have data storage access in their virtual desktop.
